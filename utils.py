...
def remove_urls(text):
    replace_by_blank = re.compile('(https|http)?:\/\/(\w|\.|\/|\?|\=|\&|\%)*\b')
    text = replace_by_blank.sub('',text)
    return(text)

def remove_users(text):
    replace_by_blank = re.compile('\@\b\w+\b')
    text = replace_by_blank.sub('\@user',text)
    return(text)

def remove_masked_users_urls(text):
    url_to_be_deleted = re.compile('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\), ]|(?:%[0-9a-fA-F][0-9a-fA-F]))+')
    text = url_to_be_deleted.sub('',text)
    replace_by_blank = re.compile('\@url|\@user|url|user')
    text = replace_by_blank.sub('',text)
    return(text)
...
def clean_text(text, language):
    ...
    replace_by_blank_symbols_2 = re.compile('\/|\(|\)|\{|\}|\[|\]|\,|\;|\.|\!|\?|\:|&amp|\n')
...
#proprocess social media post given its language
def preprocess(text, language):
    text = remove_masked_users_urls(text)
    result = []
    for token in clean_text(str(text), language).split():
        if token not in stopwords.words(language) and len(token)>1:
            result.append(token)
    return result
...
def upload_fasttext_embeddings(embeddings_path):
  ...
  dictionary = FastVector(vector_file = embeddings_path) #embeddings_path+'wiki.'+language_code+'.vec')
